...
routes.get('/', (req, res) => {
    ...
    try {
        ...
        if (type == "cancelled") {
            // Match cancelled
            res.send('302|2')
            setTimeout(cancelMatchProcedure, 3000)
        }else if(type == "ended"){
            res.send('302|1')
            setTimeout(endMatchProcedure, 1000)
        }

        async function cancelMatchProcedure() {
            ...
            if(gamemode==Gamemode.groupfight){
                io.emit("servermsg",`A battle has been cancelled on ${server}!`)
            }else if(gamemode==Gamemode.battle){
                io.emit("servermsg",`A groupfight has been cancelled on ${server}!`)
            }
            setTimeout(()=>{
        ...
        async function endMatchProcedure(){
            ...
            let playerRecords = await ActiveLobby.aggregate([
                { "$match": { "lobbyID": "server" } },
                { "$lookup": {
                  "localField": "uid",
                  "from": "playerStatus",
                  "foreignField": "uid",
                ...
                { "$project": {
                  ...
                  "hitsLanded": 1,
                  "deaths": 1,
                  "playerStatus.presetmmr": 1,
                } }
              ]);

              for (const player of playerRecords) {
                const {uid,team,username,mmr,kills,teamkills,damage,teamDamage,hitsLanded,deaths,playerStatus:{presetmmr}} = player
                let win = 0;
                let base_mmr = 0;
